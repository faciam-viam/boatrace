...
#phone-frame {
  ...
  background-image: url('https://raw.githubusercontent.com/faciam-viam/boatrace/main/imagesphone-bg.png');
...
#dashboard {
  width: 100%; height: 100%; display: flex; flex-direction: column; gap: 6px; padding: 8px 6px;
...
#player-table tbody td {
  padding: 11px 1px; text-align: center; border-bottom: 1px solid #2a3349; font-size: 8px;
}
...
def generate_dashboard_data(csv_path, output_dir="dist2_padding11"):
    ...
    for (place, round_no), group in grouped:
        ...
        bottom_bar_list.append({
            "labels": [f"{i}枠" for i in range(1, 7)],
...
def generate_combined_html(csv_path, source_dir="dist2_padding11", output_dir="dist2_padding11_combined"):
    import os
    df = pd.read_csv(csv_path)
    df.columns = df.columns.str.strip().str.replace('‐', '-').str.replace('−', '-')
    
    venues = df.groupby('レース場')
    
    for venue_name, venue_group in venues:
        html_parts = []
...
<body>""")
        ...
        for race_num in sorted(venue_group['レース回'].unique()):
            race_path = os.path.abspath(f"{source_dir}/{venue_name}_{race_num}/index.html")
            html_parts.append(f"""
<div class="race-container">
    <iframe src="file:///{race_path.replace(chr(92), '/')}"></iframe>
</div>""")
        ...
        with open(os.path.join(output_dir, f"{venue_name}.html"), "w", encoding="utf-8") as f:
            f.write("\n".join(html_parts))
        
        print(f"会場統合HTML作成: {venue_name}.html")
...
if __name__ == "__main__":
    ...
    # 会場ごとにまとめたHTMLを生成
    generate_combined_html('merged_results.csv')
    
    print("\n全ての処理が完了しました！")
    print("dist2_padding11_combined フォルダ内の会場名.htmlを開いてPDF化できます")
    generate_dashboard_data('merged_results.csv')