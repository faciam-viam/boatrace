...
st.markdown("""
    ...
    .waku-6 { border-left-color: #00ff00; }
...
with tab1:
    ...
    show_cond1_and_2 = st.sidebar.checkbox("条件1&2: 両方該当", value=True)
    ...
    if show_cond1:
        ...
        if not condition1.empty:
            condition1 = condition1.sort_values('締切予定時刻')
            
            st.markdown('<div class="player-cards-container">', unsafe_allow_html=True)
            for _, row in condition1.iterrows():
                waku = int(row['枠番']) if pd.notna(row['枠番']) else 0
                rate_pct = (row['1着率'] * 100) if pd.notna(row['1着率']) else 0
                
                st.markdown(f"""
                <div class="player-card waku-{waku}">
                    <div class="card-header">
                        <div class="card-waku">{waku}</div>
                        <div class="card-name">{row['選手名']}</div>
                        <div class="card-venue">{row['レース場']}<br>{row['レース回']}<br>{row['締切予定時刻']}</div>
                    </div>
                    <div class="card-stats">
                        <div class="stat-item">
                            <span class="stat-label">級別</span>
                            <span class="stat-value">{row['級別']}</span>
                        </div>
                        <div class="stat-item">
                            <span class="stat-label">コース1着率</span>
                            <span class="stat-value highlight-value">{rate_pct:.1f}%</span>
                        </div>
                        <div class="stat-item">
                            <span class="stat-label">全国勝率</span>
                            <span class="stat-value">{row['全国勝率']:.2f}</span>
                        </div>
                        <div class="stat-item">
                            <span class="stat-label">当地勝率</span>
                            <span class="stat-value">{row['当地勝率']:.2f}</span>
                        </div>
                    </div>
                </div>
                """, unsafe_allow_html=True)
    ...
    if show_cond2:
        ...
        if not condition2.empty:
            condition2 = condition2.sort_values('締切予定時刻')
            
            st.markdown('<div class="player-cards-container">', unsafe_allow_html=True)
            for _, row in condition2.iterrows():
                waku = int(row['枠番']) if pd.notna(row['枠番']) else 0
                deashi_total = row['出足伸び足合計'] if pd.notna(row['出足伸び足合計']) else 0
                ...
                <div class="player-card waku-{waku}">
                    ...
                    <div class="card-stats">
                        ...
                        <div class="stat-item">
                            <span class="stat-label">出足+伸び足</span>
                            <span class="stat-value highlight-value">{deashi_total:.0f}</span>
                        </div>
                        <div class="stat-item">
                            <span class="stat-label">出足</span>
                            <span class="stat-value">{row['出足']}</span>
                        </div>
                        <div class="stat-item">
                            <span class="stat-label">伸び足</span>
                            <span class="stat-value">{row['伸び足']}</span>
                        </div>
                        <div class="stat-item">
                            <span class="stat-label">全国勝率</span>
                            <span class="stat-value">{row['全国勝率']:.2f}</span>
    ...
    if show_cond1_and_2:
        ...
        if not cond1_and_2.empty:
            cond1_and_2 = cond1_and_2.sort_values('締切予定時刻')
            
            st.markdown('<div class="player-cards-container">', unsafe_allow_html=True)
            for _, row in cond1_and_2.iterrows():
                waku = int(row['枠番']) if pd.notna(row['枠番']) else 0
                rate_pct = (row['1着率'] * 100) if pd.notna(row['1着率']) else 0
                deashi_total = row['出足伸び足合計'] if pd.notna(row['出足伸び足合計']) else 0
                ...
                <div class="player-card waku-{waku}">
                    ...
                    <div class="card-stats">
                        ...
                        <div class="stat-item">
                            <span class="stat-label">出足/伸び足</span>
                            <span class="stat-value">{row['出足']}/{row['伸び足']}</span>
                        </div>
                        <div class="stat-item">
                            <span class="stat-label">全国勝率</span>
                            <span class="stat-value">{row['全国勝率']:.2f}</span>
    ...
    if show_cond3:
        ...
        if not condition3.empty:
            condition3 = condition3.sort_values('締切予定時刻')
            
            st.markdown('<div class="player-cards-container">', unsafe_allow_html=True)
            for _, row in condition3.iterrows():
                waku = int(row['枠番']) if pd.notna(row['枠番']) else 0
                ...
                <div class="player-card waku-{waku}">
                    ...
                    <div class="card-stats">
                        ...
                        <div class="stat-item">
                            <span class="stat-label">伸び足</span>
                            <span class="stat-value highlight-value">{row['伸び足']}</span>
                        </div>
                        <div class="stat-item">
                            <span class="stat-label">出足</span>
                            <span class="stat-value">{row['出足']}</span>
            ...
            for _, row in df4.iterrows():
                ...
                <div class="player-card waku-{waku}">
                    ...
                    <div class="card-stats">
                        ...
                        <div class="stat-item">
                            <span class="stat-label">レース内ST順位</span>
                            <span class="stat-value highlight-value">{row['レース内ST順位']}</span>
                        </div>
                        <div class="stat-item">
                            <span class="stat-label">コース平均ST</span>
                            <span class="stat-value">{row['コース平均st']:.2f}</span>
                        </div>
                        <div class="stat-item">
                            <span class="stat-label">全国勝率</span>
                            <span class="stat-value">{row['全国勝率']:.2f}</span>
    ...
    if show_cond5:
        ...
        if pickup5_list:
            ...
            for _, row in df5.iterrows():
                st.markdown(f"""
                <div class="player-card waku-4">
                    <div class="card-header">
                        <div class="card-waku">4</div>
                        <div class="card-name">{row['4枠選手']}</div>
                        <div class="card-venue">{row['レース場']}<br>{row['レース回']}<br>{row['締切予定時刻']}</div>
                    </div>
                    <div class="card-stats">
                        <div class="stat-item">
                            <span class="stat-label">3枠選手</span>
                            <span class="stat-value">{row['3枠選手']}</span>
                        </div>
                        <div class="stat-item">
                            <span class="stat-label">ST差</span>
                            <span class="stat-value highlight-value">{row['ST差']:.2f}</span>
                        </div>
                        <div class="stat-item">
                            <span class="stat-label">出足差</span>
                            <span class="stat-value highlight-value">{row['出足差']:.0f}</span>
                        </div>
                        <div class="stat-item">
                            <span class="stat-label">4枠ST</span>
                            <span class="stat-value">{row['4枠ST']:.2f}</span>
                        </div>
                        <div class="stat-item">
                            <span class="stat-label">4枠出足+伸び足</span>
                            <span class="stat-value">{row['4枠出足+伸び足']:.0f}</span>
                        </div>
                    </div>
                </div>
                """, unsafe_allow_html=True)
    ...
    if show_cond6:
        ...
        if pickup6_list:
            ...
            for _, row in df6.iterrows():
                st.markdown(f"""
                <div class="player-card waku-5">
                    <div class="card-header">
                        <div class="card-waku">5</div>
                        <div class="card-name">{row['5枠選手']}</div>
                        <div class="card-venue">{row['レース場']}<br>{row['レース回']}<br>{row['締切予定時刻']}</div>
                    </div>
                    <div class="card-stats">
                        <div class="stat-item">
                            <span class="stat-label">3枠</span>
                            <span class="stat-value">{row['3枠選手']}</span>
                        </div>
                        <div class="stat-item">
                            <span class="stat-label">4枠</span>
                            <span class="stat-value">{row['4枠選手']}</span>
                        </div>
                        <div class="stat-item">
                            <span class="stat-label">5枠ST</span>
                            <span class="stat-value highlight-value">{row['5枠ST']:.2f}</span>
                        </div>
                        <div class="stat-item">
                            <span class="stat-label">5枠出足+伸び足</span>
                            <span class="stat-value highlight-value">{row['5枠出足+伸び足']:.0f}</span>
                        </div>
                    </div>
                </div>
                """, unsafe_allow_html=True)
    ...
    if show_summary:
        ...
        if not multi_pickup.empty:
            ...
            multi_pickup = multi_pickup.sort_values('締切予定時刻')
            
            st.markdown('<div class="player-cards-container">', unsafe_allow_html=True)
            for _, row in multi_pickup.iterrows():
                waku = int(row['枠番']) if pd.notna(row['枠番']) else 0
                rate_pct = row['コース1着率%'] if pd.notna(row['コース1着率%']) else 0
                ...
                <div class="player-card waku-{waku}">
                    ...
                    <div class="card-stats">
                        ...
                        <div class="stat-item">
                            <span class="stat-label">条件カウント</span>
                            <span class="stat-value highlight-value">{row['条件カウント']:.0f}</span>
                        </div>
                        <div class="stat-item">
                            <span class="stat-label">コース1着率</span>
                            <span class="stat-value">{rate_pct:.1f}%</span>
                        </div>
                        <div class="stat-item">
                            <span class="stat-label">出足+伸び足</span>
                            <span class="stat-value">{row['出足伸び足合計']:.0f}</span>
                        </div>
                        <div class="stat-item">
                            <span class="stat-label">全国勝率</span>
                            <span class="stat-value">{row['全国勝率']:.2f}</span>
