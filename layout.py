...
try:
    ...
    if df.empty:
        ...
        # 締切予定時刻でソート
        df_sorted = df.sort_values('締切予定時刻').copy()
        
        # レース一覧を作成
        race_list = []
        for (venue, race_num), race_df in df_sorted.groupby(['レース場', 'レース回'], sort=False):
            race_time = str(race_df.iloc[0]['締切予定時刻']) if '締切予定時刻' in race_df.columns else ''
            race_list.append({
                'venue': venue,
                'race_num': race_num,
                'race_time': race_time,
        ...
        # 全レース表示（24レース単位で選択可能に）
        total_races = len(race_list)
        
        if total_races <= 24:
            # 24レース以下ならそのまま全表示
            for i, race_info in enumerate(race_list):
                race_df = race_info['data'].copy()
                race_df['w_num'] = pd.to_numeric(race_df['枠番'], errors='coerce').fillna(0)
                race_df = race_df.sort_values('w_num').reset_index(drop=True)
                
                render_race(race_df, race_info['venue'], race_info['race_num'], key_prefix=f"race_{i}")
                st.markdown("---")
        else:
            # 24レースごとに区切って選択肢を作成
            time_ranges = []
            for i in range(0, total_races, 24):
                end_idx = min(i + 24, total_races)
                group = race_list[i:end_idx]
                start_time = group[0]['race_time']
                end_time = group[-1]['race_time']
                time_ranges.append({
                    'label': f"{start_time} ~ {end_time} ({len(group)}レース)",
                    'start_idx': i,
                    'races': group
                })
            ...
            selected_time = st.sidebar.radio("時間帯選択", time_labels)
            
            # 選択された時間帯のレースを表示
            selected_range = next(tr for tr in time_ranges if tr['label'] == selected_time)
            
            for i, race_info in enumerate(selected_range['races']):
                race_df = race_info['data'].copy()
                race_df['w_num'] = pd.to_numeric(race_df['枠番'], errors='coerce').fillna(0)
                race_df = race_df.sort_values('w_num').reset_index(drop=True)
                
                render_race(race_df, race_info['venue'], race_info['race_num'], key_prefix=f"race_{selected_range['start_idx']}_{i}")
                st.markdown("---")
